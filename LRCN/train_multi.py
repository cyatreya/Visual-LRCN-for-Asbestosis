...
parser.add_argument('--latent_dim', default=512, type=int,
                    help='The dim of the Conv FC output (default:512)')
...
parser.add_argument('--checkpoint_interval', default=5,
                    type=int, help='Interval between saving model checkpoints')
parser.add_argument('--val_check_interval', default=5,
                    type=int, help='Interval between running validation test')
...
parser.add_argument('--number_of_classes', default=2, type=int,
                    help='The number of classes we would train on')
...
>>>>>>> c139c9917a10d56fee85a67d8fbe6c84dfd80b13
    ...
    if args.load_all_data_to_RAM:
        dataloaders = load_all_dataset_to_RAM(
            dataloaders, dataset_order, args.batch_size)
    plot_label_distribution(dataloaders, folder_dir,
                            args.load_all_data_to_RAM, label_decoder_dict)
    print('Data prepared\nLoading model...')
    num_class = len(
        label_decoder_dict) if args.number_of_classes is None else args.number_of_classes
...
>>>>>>> c139c9917a10d56fee85a67d8fbe6c84dfd80b13
    ...
    for epoch in range(args.epochs):
        ...
        train_loss, train_acc = train_model(
            model, dataloaders['train'], device, optimizer, criterion)
        if (epoch % args.val_check_interval) == 0:
            val_loss, val_acc, predicted_labels, images = test_model(model, dataloaders['val'], device,
                                                                     criterion)
            plot_images_with_predicted_labels(
                images, label_decoder_dict, predicted_labels, folder_dir, epoch)
            end_epoch = time.time()
        ...
        if (epoch % args.checkpoint_interval) == 0:
            ...
            torch.save(hp_dict, os.path.join(
                save_model_dir, 'epoch_{}.pth.tar'.format(epoch)))
